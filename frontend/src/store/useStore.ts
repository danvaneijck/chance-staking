...
interface AppState extends WalletState, BalanceState, ContractState, UserState, DrawsState {
  isLoading: boolean
  error: string | null
  ...
  persist(
    (set, get) => ({
      ...
      fetchContractData: async () => {
        if (!CONTRACTS.stakingHub || !CONTRACTS.rewardDistributor) return
        try {
          ...
          set({
            ...
            totalRewardsDistributed: drawState.total_rewards_distributed,
          })
        } catch (err: any) {
        ...
        try {
          const [unstakeReqs, userWins, winDraws] = await Promise.all([
            ...
            CONTRACTS.rewardDistributor
              ...
              : null,
            CONTRACTS.rewardDistributor
              ? contracts.fetchUserWinDetails(injectiveAddress)
              : [],
          ])
          set({
      ...
      fetchDraws: async () => {
        ...
        try {
          const { draws } = await contracts.fetchDrawHistory(undefined, 20)
      ...
      stake: async (amount: string) => {
        ...
        try {
          const msg = contracts.buildStakeMsg(injectiveAddress, amount)
      ...
      unstake: async (amount: string) => {
        ...
        set({ isLoading: true, error: null })
        try {
          const msg = contracts.buildUnstakeMsg(injectiveAddress, amount)
          await msgBroadcaster.broadcast({
            msgs: [msg],
            injectiveAddress,
          })
